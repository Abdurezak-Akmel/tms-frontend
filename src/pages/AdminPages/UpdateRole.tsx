...
const UpdateRole = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                // Fetch Role Details
                const roleResponse = await roleService.getRoleById(Number(id));
                if (roleResponse.success && roleResponse.role) {
                    setRoleName(roleResponse.role.role_name);
                    setDescription(roleResponse.role.description || '');
                }

                // Fetch Assigned Courses
                const assignedResponse = await roleCourseService.getCoursesByRoleId(Number(id));
                if (assignedResponse.success && assignedResponse.courses) {
                    const assignedIds = assignedResponse.courses.map((c: any) => c.course_id);
    ...
    const handleToggleCourse = (courseId: number) => {
        setSelectedCourseIds((prev: number[]) =>
            prev.includes(courseId)
                ? prev.filter((id: number) => id !== courseId)
                : [...prev, courseId]
        );
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const roleResponse = await roleService.updateRole(roleId, {
                role_name: roleName,
                description: description
            });

            if (roleResponse.success) {
                // 2. Diff courses and update assignments
                const coursesToRemove = initialCourseIds.filter(cid => !selectedCourseIds.includes(cid));
                const coursesToAdd = selectedCourseIds.filter(cid => !initialCourseIds.includes(cid));

                const promises = [];

                if (coursesToRemove.length > 0) {
                    promises.push(roleCourseService.removeMultipleCoursesFromRole({
                        role_id: roleId,
                        course_ids: coursesToRemove
                    }));
                }

                if (coursesToAdd.length > 0) {
                    promises.push(roleCourseService.assignMultipleCoursesToRole({
                        role_id: roleId,
                ...
                setError(roleResponse.error || 'Failed to update role');
            }
        } catch (err: any) {
    ...
    if (initialLoading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
                ...
                <p className="text-slate-500 font-medium">Loading role details...</p>
            </div>
        );
    }
    ...
    return (
        <Stack gap="lg" className="pb-20 max-w-5xl mx-auto">
            <PageHeader
                title="Update Role"
                description={`Edit the permissions and details for the "${roleName}" role.`}
                        ...
                        <div className="bg-blue-50/50 border border-blue-100/50 p-4 rounded-2xl">
                            <div className="flex gap-3 text-blue-700">
                                ...
                                <p className="text-xs leading-relaxed font-medium">
                                    Updating a role will immediately affect the permissions of all users currently assigned to this role.
                                </p>
                            </div>
                        </div>
                    </div>
